...
const handleRenderList = () => {
  return data.default.map((item, index) => {
    return (<div key={index}>
      <ul>
        <h5>{item.title}</h5>
        {handleRenderListItem(item.index)}
      </ul>
    </div>
    )
  })
...
const handleRenderListItem = (item) => {
  return item.map((listItem, index) => {
    return (
      <li key={index} className="dropdown-item">{listItem}</li>
    )
  })
}
    ...
    <div className='my-5'>
      ...
      <div className='d-flex justify-content-between align-items-center my-3'>
        <div className='d-flex align-items-center'>
          ...
          <span className='ml-3'>© Fiverr International Ltd. 2022</span>
        </div>
        <div className='d-flex align-items-center'>
          <div>
            <i className="fa fa-twitter p-1"></i>
            <i className="fa fa-facebook p-1"></i>
            <i className="fa fa-linkedin p-1"></i>
            <i className="fa fa-pinterest p-1"></i>
            <i className="fa fa-instagram p-1"></i>
          </div>
          <div>
            <span className='btn btn-success'><i className="fa fa-globe"></i> English</span>
            <span className='btn btn-success'>US$ USD</span>
            <span>
